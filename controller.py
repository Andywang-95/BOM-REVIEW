...
class SubWindow_controller(QtWidgets.QMainWindow):
    ...
    def state_keep(self):
        print(self.ui.path_check.isChecked(),self.ui.pn_check.isChecked(),self.ui.firstcol_check.isChecked())
...
class MainWindow_controller(QtWidgets.QMainWindow):
    ...
    def preview_setting(self):
        if self.ui.custom_radio.isChecked() :
            ...
            self.ui.pn_text.setText(dict_setting['pn_col']+str(dict_setting['fir_col']))
        else:
    ...
    def add_substitute_review(self):
        print('tiptop')
        try:
            if func_w.check_file(data['database_path']) == None:
                pass
            else:
                self.ui.display_1.append(func_w.check_file(data['database_path']))
                return
            wb_mapping,ws_mapping = func_w.load('%s/mapping.xlsx'%(data['database_path']))
            dict_mapping = func_w.to_dict(ws_mapping)
            p = self.ui.bompath_text_1.toPlainText()
            p = self.format_change(p)
        
            wb_bom,ws_bom = func_w.load(p)
            ...
            for action,num,comment in zip(ws_bom['A'][data['fir_col']-1:],
                ...
                if type(num.value) != str:
                    ...
                    if action.value == 'Add Substitute':
                        ...
                        if num.value in dict_mapping:
                            ...
                            if dict_mapping[num.value][2].value == previous and dict_mapping[num.value][2].value.upper() != 'AGREE':
                                comment.value = '同上'
            ...
            self.ui.display_1.append('Error:\n' + str(traceback.format_exc()) + '\n-------------------------------------------------------------------------------')
    ...
    def sys_bom_review(self):
        ...
        try:
            ...
            previous = ''
            print(ws_bom[data['pn_col']][data['fir_col']-1].value)
            for action,num,comment in zip(ws_bom['A'][2:],
                                          ws_bom['C'][2:],
                                          ws_bom[l][2:]):
                if type(num.value) != str:
                    continue
    ...
    def result_bom_review(self):
        ...
        try:
            ...
            for cell in ws_bom[l]:
                ...
                cell.fill = fill
            others = []
            for num,comment in zip(ws_bom['B'],
                                   ws_bom[l]):
                if type(num.value) != str:
                    continue
                elif len(num.value) != 16:
    ...
    def custom_bom_review(self):
        print('custom')
        try:
            if func_w.check_file(dict_setting['database_path']) == None:
                pass
            else:
                self.ui.display_1.append(func_w.check_file(dict_setting['database_path']))
                return
            wb_mapping,ws_mapping = func_w.load('%s/mapping.xlsx'%(dict_setting['database_path']))
            dict_mapping = func_w.to_dict(ws_mapping)
            p = self.ui.bompath_text_1.toPlainText()
            ...
            for cell in ws_bom[l]:
                ...
                cell.fill = fill
            others = []
            for num,comment in zip(ws_bom[dict_setting['pn_col']][dict_setting['fir_col']-1:],
    ...
    def run_import(self):
        try:
            if func_w.check_file(data['database_path']) == None:
                ...
                self.ui.display_2.append(func_w.check_file(data['database_path']))
                return
            wb_mapping,ws_mapping = func_w.load('%s/mapping.xlsx'%(data['database_path']))
            wb_mapping.save('mapping import-backup.xlsx')

            path = self.ui.bompath_text_2.toPlainText()
            wb_bom,ws_bom = func_w.load(path)
            ...
            for a,b,c,d in zip(ws_bom[data['pn_col']][data['fir_col']-1:],
                    ...
                    if d.value != dict_mapping[a.value][2].value:
                        if d.value == '同上':
                            if dict_mapping[a.value][2].value != previous:
                                ws_dup.append([a.value,previous,dict_mapping[a.value][2].value])
                                dup_count += 1
                                continue
                            else:
                                continue
                        else:
                            ws_dup.append([a.value,d.value,dict_mapping[a.value][2].value])
                            dup_count += 1
                            previous = d.value
                            continue
                else:
            ...
            if dup_count>0:
                wb_compare.save('compare(%s).xlsx'%(path.split('/')[-1].split('.')[0]))

            wb_maintain,ws_maintain = func_w.load('%s/maintain.xlsx'%(data['database_path']))
            wb_maintain.save('maintain-backup.xlsx')

            c = func_w.to_maintain(wb_maintain,dict_add)
            ...
            self.ui.display_2.append('Error:\n' + str(traceback.format_exc()) + '\n-------------------------------------------------------------------------------')
    ...
    def run_update(self):
        try:
            if func_w.check_file(data['database_path']) == None:
                ...
                self.ui.display_3.append(func_w.check_file(data['database_path']))
                return
            wb_mapping,ws_mapping = func_w.load('%s/mapping.xlsx'%(data['database_path']))
            ...
            for name in ['機構料件','電子料(1)','電子料(2)','電子料(R,C)','Others']:
                ...
                print(name)
                wb_maintain,ws_maintain = func_w.load('%s/maintain.xlsx'%(data['database_path']),name)
                dict_maintain = func_w.to_dict(ws_maintain)
                dict_mapping = func_w.to_dict(ws_mapping)
                
                for i in dict_maintain:
                    if i in dict_mapping:
                        if dict_mapping[i][2].value != dict_maintain[i][2].value:
                            upd.append([i,dict_mapping[i][2].value,dict_maintain[i][2].value])
                            dict_mapping[i][2].value = dict_maintain[i][2].value
                            

                        if dict_mapping[i][2].fill.start_color.rgb != dict_maintain[i][2].fill.start_color.rgb or dict_mapping[i][2].font != copy(dict_maintain[i][2].font):
                        ...
                        ws_mapping.append([i,dict_maintain[i][0].value,dict_maintain[i][1].value,dict_maintain[i][2].value])
                ...
                if len(new) != 0:
                    ...
                    # df.index=df.index+1
                    self.ui.display_3.append(tabulate(new, headers=['PartNum','Comment'], tablefmt = 'html'))
                ...
                if len(upd) != 0:
                    ...
                    # df.index=df.index+1
                    self.ui.display_3.append(tabulate(upd, headers=['PartNum','Old_Comment','New_Comment'], tablefmt = 'html'))

                if len(color) != 0:
                    self.ui.display_3.append('\n更新High Light %s 筆資料\n'%len(color))
                    # df = pd.DataFrame(color,columns = ['PartNum','Comment'])
                    # df.index=df.index+1
                    self.ui.display_3.append(tabulate(color, headers=['PartNum','Comment'], tablefmt = 'html'))
            ...
            self.ui.display_3.append('Error:\n' + str(traceback.format_exc()) + '\n-------------------------------------------------------------------------------')
